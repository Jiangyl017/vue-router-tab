...
const RouterTab = {
  ...
  methods: {
    ...
    presetTabs(tabs = this.tabs) {
      ...
      newTabs = this.onChange(newTabs) || newTabs
      this.items = newTabs
    },

    // RouterAlive 缓存更新时同步更改页签
    onAliveChange(type, matched) {
      const { items: oldTabs, lastMatchedKey } = this
      const items = [...oldTabs]
      const { key } = matched
      ...
      if (matchIdx > -1) {
        ...
        if (type === 'replace') {
          ...
          if (items.length === 0) {
            ...
            const oldIdx = items.findIndex(({ id }) => id === matched.oldKey)
            items[oldIdx] = item
            // 清除缓存
            this.$alive.remove(matched.oldKey)
          }
        } else {
          // 新增页签
      ...
      const newTabs = this.onChange(items) || items
      this.items = newTabs
    },
